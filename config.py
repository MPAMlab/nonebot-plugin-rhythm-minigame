...
class Config(BaseSettings, extra=Extra.ignore):
    rhythm_thing: str = "面包"  # 自定义物品，改掉默认的”面包“ 此为全局
    ...
    level_rhythm_num: int = 10  # 设置升一级所需要的面包数量（数据库不保存等级！！等级会随之而变！）
...
class CD(Enum):
    """操作冷却（单位：秒）"""
    BUY = rhythm_config.cd_buy
    EAT = rhythm_config.cd_eat
    ROB = rhythm_config.cd_rob
    GIVE = rhythm_config.cd_give
    BET = rhythm_config.cd_bet
...
def random_config():
    ...
    special_settings = [rhythm_config.special_buy_group, rhythm_config.special_eat_group, rhythm_config.special_rob_group,
                        rhythm_config.special_give_group, rhythm_config.special_bet_group]
