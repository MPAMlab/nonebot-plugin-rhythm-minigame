...
def buy_event_gold_rhythm(event: BuyEvent):
    buy_num = event.action_num + 20
    new_rhythm_num = event.rhythm_db.add_rhythm(event.user_id, buy_num)
    new_rhythm_no = event.rhythm_db.update_no(event.user_id)
    append_text = f"出现了！黄金{event.thing}！计为{buy_num}个！现在一共拥有{new_rhythm_num}个{event.thing}！" \
                  f"您的{event.thing}排名为:{new_rhythm_no}"
    event.rhythm_db.cd_update_stamp(event.user_id, Action.BUY)
    return append_text


@probability(0.2, Action.BUY, priority=5)
def buy_event_poverty_relief(event: BuyEvent):
    if event.user_data.rhythm_num > 10:
        return
    buy_num = event.action_num + 10
    new_rhythm_num = event.rhythm_db.add_rhythm(event.user_id, buy_num)
    new_rhythm_no = event.rhythm_db.update_no(event.user_id)
    append_text = f"{event.thing}店看你{event.thing}太少了，送了你{buy_num}个！现在一共拥有{new_rhythm_num}个{event.thing}！" \
                  f"您的{event.thing}排名为:{new_rhythm_no}"
    event.rhythm_db.cd_update_stamp(event.user_id, Action.BUY)
    return append_text
...
def buy_event_too_much(event: BuyEvent):
    if event.user_data.rhythm_num < 90:
        return
    append_text = f"你{event.thing}太多啦！不卖给你了，快吃！现在一共拥有{event.user_data.rhythm_num}个{event.thing}！" \
...
def eat_event_too_much_rhythm(event: EatEvent):
    if event.user_data.rhythm_num < 90:
        return
    txt = event.normal_event()
    txt += f"\n你{event.thing}太多啦，我允许你再吃一次吧！"
    event.rhythm_db.cd_refresh(event.user_id, Action.EAT)
    return txt
...
def eat_event_not_enough(event: EatEvent):
    ...
    event.rhythm_db.reduce_rhythm(event.user_id, eat_num)
    event.rhythm_db.add_rhythm(event.user_id, eat_num, Action.EAT)
    append_text = f"成功吃掉了{eat_num}个{event.thing}w！还是好饿，还可以继续吃！"
    event.rhythm_db.update_no(event.user_id)
    return append_text


@probability(0.1, Action.EAT, priority=5)
def eat_event_much(event: EatEvent):
    if event.user_data.rhythm_num <= MAX.EAT.value * 2:
        return
    ...
    event.rhythm_db.update_no(event.user_id)
    event.rhythm_db.cd_ban_action(event.user_id, Action.EAT, 1800)
    return append_text
...
def eat_event_refresh(event: EatEvent):
    ...
    event.rhythm_db.update_no(event.user_id)
    event.rhythm_db.cd_refresh(event.user_id, Action.ALL)
...
def eat_event_good(event: EatEvent):
    ...
    event.rhythm_db.update_no(event.user_id)
    event.rhythm_db.add_rhythm(event.user_id, LEVEL, Action.EAT)
    return append_text


@probability(0.015, Action.EAT, priority=5)
def eat_event_bad(event: EatEvent):
    if event.user_data.rhythm_eaten <= 10:
        return
    append_text = f"{event.thing}坏啦！吃了一个坏{event.thing}！难受死了，等级减1。"
    event.rhythm_db.reduce_rhythm(event.user_id, 1)
    event.rhythm_db.reduce_rhythm(event.user_id, LEVEL, Action.EAT)
    event.rhythm_db.update_no(event.user_id)
    return append_text
...
def eat_event_rob(event: EatEvent):
    ...
    event.rhythm_db.cd_refresh(event.user_id, Action.ROB)
    event.rhythm_db.cd_update_stamp(event.user_id, Action.EAT)
    return append_text


@probability(0.025, Action.EAT, priority=5)
def eat_event_find_bad(event: EatEvent):
    if event.user_data.rhythm_num < MAX.EAT.value:
        return
    bad_num = event.action_num
...
def rob_event_fail(event: RobEvent):
    loss_num = random.randint(0, min(MAX.ROB.value, event.user_data.rhythm_num))
    new_rhythm_num = event.rhythm_db.reduce_rhythm(event.user_id, loss_num)
    event.rhythm_db.update_no(event.user_id)
    append_text = f"抢{event.thing}失败啦！被{event.other_name}反击，你丢失{loss_num}个{event.thing}！" \
                  f"你现在拥有{new_rhythm_num}个{event.thing}！"
    event.rhythm_db.cd_update_stamp(event.user_id, Action.ROB)
    return append_text
...
def rob_event_police(event: RobEvent):
    ...
    event.rhythm_db.cd_ban_action(event.user_id, Action.ROB, 2400)
    return append_text
...
def rob_event_police2(event: RobEvent):
    if event.user_data.rhythm_num < event.action_num:
        return
    loss_num = event.action_num
    append_text = f"你抢{event.thing}被我抓住了！你真的太坏了！我要罚你{loss_num}个{event.thing}！"
    event.rhythm_db.reduce_rhythm(event.user_id, loss_num)
    event.rhythm_db.update_no(event.user_id)
    event.rhythm_db.cd_update_stamp(event.user_id, Action.ROB)
    return append_text
...
def rob_event_much(event: RobEvent):
    if event.other_data.rhythm_num < event.action_num * 2:
        return
    rob_num = event.action_num * 2
    append_text = f"抢劫大成功！抢了{event.other_name}{rob_num}个{event.thing}！"

    event.rhythm_db.add_rhythm(event.user_id, rob_num)
    event.rhythm_db.reduce_rhythm(event.other_id, rob_num)
    event.rhythm_db.update_no(event.user_id)
    event.rhythm_db.update_no(event.other_id)
...
def give_event_commission(event: GiveEvent):
    if event.user_data.rhythm_num <= MAX.GIVE.value * 2:
        return
    give_num = event.action_num
    user_num = event.rhythm_db.reduce_rhythm(event.user_id, give_num * 2)
    event.rhythm_db.add_rhythm(event.other_id, give_num)
    append_text = f"哇！这么多{event.thing}，你送了{give_num}个给{event.other_name}！" \
                  f"再给我{give_num}个吧嘿嘿！你现在有{user_num}个{event.thing}！"
    event.rhythm_db.update_no(event.user_id)
    event.rhythm_db.update_no(event.other_id)
    event.rhythm_db.cd_update_stamp(event.user_id, Action.GIVE)
    return append_text
...
def give_event_lossless2(event: GiveEvent):
    give_num = event.action_num
    give_num2 = random.randint(MIN.GIVE.value, min(MAX.GIVE.value, event.user_data.rhythm_num))
    event.rhythm_db.add_rhythm(event.other_id, give_num)
    event.rhythm_db.add_rhythm(event.user_id, give_num2)
...
def bet_event_addiction(event: BetEvent):
    ...
    event.rhythm_db.cd_refresh(event.user_id, Action.BET)
    return append_text
...
def bet_event_police(event: BetEvent):
    ...
    event.rhythm_db.cd_ban_action(event.user_id, Action.BET, 2400)
    return append_text
...
def bet_event_cheat(event: BetEvent):
    ...
    event.rhythm_db.reduce_rhythm(event.user_id, loss_num)
    event.rhythm_db.update_no(event.user_id)
    event.rhythm_db.cd_update_stamp(event.user_id, Action.BET)
    return append_text


@probability(0.04, Action.BET, priority=5)
def bet_event_much(event: BetEvent):
    if event.user_data.rhythm_num < event.action_num * 2:
        return
